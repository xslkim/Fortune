...
export function sphere(r = 1.6, wseg = 32, hseg = 24) {
  ...
  for (let j = 1; j < hseg; j++) {
    ...
    for (let i = 0; i < wseg; i++) {
      ...
      points.push(
        pt(r * Math.sin(phi) * Math.cos(t), r * Math.cos(phi), r * Math.sin(phi) * Math.sin(t), ''),
      );
    }
  }
  ...
  for (let i = 0; i < wseg; i++)
    faces.push({ idx: [at(hseg - 1, i), at(hseg - 1, i + 1), south], smooth: true });
  // 球心移到原点
