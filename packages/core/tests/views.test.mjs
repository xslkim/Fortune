...
test('圆柱：正视图是长方形（无斜线/弧线），俯视图是近似圆的折线', () => {
  ...
  for (const s of f.segments) {
    const dx = Math.abs(s.a[0] - s.b[0]),
      dy = Math.abs(s.a[1] - s.b[1]);
...
test('圆锥正视图是等腰三角形（底边水平 + 两条轮廓母线）', () => {
  ...
  const slant = f.segments.filter(
    (s) => Math.abs(s.a[0] - s.b[0]) > 1e-9 && Math.abs(s.a[1] - s.b[1]) > 1e-9,
  );
