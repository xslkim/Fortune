...
test('正多面体：顶点/棱/面数正确且满足欧拉公式 V-E+F=2', () => {
  for (const s of PLATONIC) {
    ...
    assert.ok(
      edges.every((e) => e.faces.length === 2 && !e.smooth),
      `${s.id} 每条棱恰属于两个面`,
    );
  }
});
...
test('正多面体：labels 符合约定', () => {
  const t = tetrahedron();
  assert.deepEqual(
    t.points.map((q) => q.label),
    ['A', 'B', 'C', 'P'],
  );
  const o = octahedron();
  assert.deepEqual(
    o.points.map((q) => q.label),
    ['A', 'B', 'C', 'D', 'P1', 'P2'],
  );
...
test('beauty.js 在无 DOM 的 node 里可 import，且卡片音频 id 都在台词表中', async () => {
  // beauty.js 是 web 端 UI 模块，测试借工作区路径跨包引用
  const m = await import('../../../apps/web/src/ui/beauty.js');
  ...
  for (const card of m.BEAUTY_CARDS) {
    ...
    const audios = [
      ...(Array.isArray(card.audio) ? card.audio : card.audio ? [card.audio] : []),
      ...(card.extraAudios || []),
    ];
