...
<template>
  <view class="geo-wrap" :style="{ height: height + 'px' }">
    ...
    ></canvas>
    <view v-if="failed" class="geo-fail">
      <text class="geo-fail-text">3D 加载失败</text>
      <button class="geo-fail-btn" size="mini" @click="retry">点击重试</button>
    </view>
    <view
      v-for="l in labels"
      :key="l.key"
...
export default {
  ...
  methods: {
      ...
      uni
        ...
        .exec((res) => {
          ...
          try {
            ...
            console.warn('[geo-canvas] 3D 初始化失败', err);
            this.failed = true;
          }
        });
    },
    retry() {
      ...
      this.labels = [];
      // failed=true 时 canvas 被 v-if 移除；先复位 failed 让 canvas 重渲染，
      // 再等原生节点可用后取 node（同 mounted 的等待）
      this.failed = false;
      this.$nextTick(() => setTimeout(() => this._init(), 60));
    },
    onTouchStart(e) {
