...
export function createThreeViewPane(solid) {
  ...
  for (const key of ['front', 'side', 'top']) {
    ...
    for (const s of view.segments) {
      const attrs = {
        x1: mapX(s.a[0]).toFixed(4), y1: mapY(s.a[1]).toFixed(4),
        x2: mapX(s.b[0]).toFixed(4), y2: mapY(s.b[1]).toFixed(4),
    ...
    const title = svgEl('text', {
      ...
      'text-anchor': 'middle', 'font-size': Math.max(span * 0.09, 0.32).toFixed(4),
      fill: COLORS.title,
    });
