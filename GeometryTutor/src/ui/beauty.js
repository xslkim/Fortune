...
import { cube, tetrahedron, octahedron, icosahedron, dodecahedron } from '../geo/solids.js';

// ---------- 样式（自动注入；也可由接线人取 BEAUTY_CSS 自行放入 <style>） ----------

export const BEAUTY_CSS = `
.beauty-root { font-size: 16px; color: #1f2d3d; }
.beauty-hero { text-align: center; padding: 18px 12px 6px; }
.beauty-hero h2 { font-size: 24px; margin: 0 0 8px; }
.beauty-hero p { color: #5a6b80; font-size: 14px; line-height: 1.8; margin: 0 auto; max-width: 560px; }
.beauty-card {
  background: #fff; border: 1px solid #dde4ee; border-radius: 14px;
  padding: 16px 18px; margin: 14px 0;
}
.beauty-card h3 { font-size: 18px; margin: 0; color: #1f3a5f; }
...
.beauty-chip.on { background: #2f6fdd; border-color: #2f6fdd; color: #fff; font-weight: 600; }
.beauty-hint { font-size: 13px; color: #8a97a8; margin-top: 6px; }
.beauty-table { width: 100%; border-collapse: collapse; margin: 10px 0 4px; font-size: 14px; }
.beauty-table th, .beauty-table td { border: 1px solid #dde4ee; padding: 8px 6px; text-align: center; }
.beauty-table th { background: #f4f7fb; color: #5a6b80; font-weight: 600; }
.beauty-table tbody tr { cursor: pointer; }
.beauty-table tbody tr:hover { background: #f0f5fc; }
.beauty-table td.beauty-ok { color: #2d6a4f; font-weight: 600; }
.beauty-voice {
  min-height: 32px; padding: 0 12px; font-size: 13px; border-radius: 99px;
  border: 1px solid #c3cfdf; background: #f4f7fb; color: #1f3a5f; cursor: pointer;
}
.beauty-actions { display: flex; align-items: center; gap: 10px; margin-top: 12px; flex-wrap: wrap; }
.beauty-share {
  min-height: 40px; padding: 0 16px; font-size: 14px; border-radius: 10px;
  border: 1px solid #2f6fdd; background: #2f6fdd; color: #fff; cursor: pointer;
}
.beauty-view3d {
  min-height: 40px; padding: 0 16px; font-size: 14px; border-radius: 10px;
  border: 1px solid #c3cfdf; background: #fff; color: #2f6fdd; cursor: pointer;
}
.beauty-toast { font-size: 13px; color: #2d6a4f; opacity: 0; transition: opacity .2s; }
...
.beauty-footer { text-align: center; padding: 8px 12px 20px; color: #5a6b80; font-size: 14px; line-height: 1.8; }
...
export const PLATONIC_FIVE = [
  ...
  { id: 'dodecahedron', name: '正十二面体', make: dodecahedron, V: 20, E: 30, F: 12, face: '正五边形 ×12' },
  { id: 'icosahedron', name: '正二十面体', make: icosahedron, V: 12, E: 30, F: 20, face: '正三角形 ×20' },
];
...
export const BEAUTY_HERO = {
  ...
  audio: 'beauty-1',
  share: '发现一个宝藏几何小站：正多面体全宇宙只有 5 种、足球是削了角的正二十面体、黄金分割直接藏在坐标里……每条都能转着 3D 模型看，特别适合给孩子磨几何直觉。',
};
...
export const BEAUTY_CARDS = [
  {
    ...
    share: '你知道吗？正多面体全宇宙只有 5 种：正四、正六（正方体）、正八、正十二、正二十面体。因为每个顶点处内角和必须小于 360°，凑来凑去只有这 5 种凑法。数学的极致简洁，转一转 3D 模型就懂。',
  },
  {
    id: 'euler',
    type: 'euler',
    ...
    share: '一个公式收编所有凸多面体：顶点数 − 棱数 + 面数 = 2。正方体 8−12+6=2，正二十面体 12−30+20=2。数学的美，就是用一行式子管无限多个形状。',
  },
  {
    id: 'soccer',
    type: 'solid',
    ...
    share: '足球为什么是五边形加六边形？它其实是把正二十面体的 12 个尖角削掉后的样子：20 个三角形变六边形，12 个切口是五边形。连 C60 分子都长这样。几何，藏在每个孩子的脚下。',
  },
  {
    id: 'golden',
    type: 'solid',
    ...
    share: '正十二面体的顶点坐标里藏着黄金分割 φ≈1.618，因为正五边形的对角线恰好是边长的 φ 倍。两千年前柏拉图说它代表宇宙。美，原来可以被写进坐标。',
  },
  {
    id: 'penrose',
    type: 'penrose',
    ...
    share: '这个三角形在三维世界根本造不出来，但你的眼睛信了。彭罗斯三角：局部都合理，整体不可能。几何课教的不只是算题，更是识破「眼见为实」的思维训练。',
  },
];
...
function createSession(container, deps) {
  ...
  function voiceButtons(ids, host) {
    for (const id of ids) {
      const b = el('button', 'beauty-voice', '🔊 听讲解');
  ...
  function renderEuler(card) {
    ...
    const thead = el('tr');
    for (const h of ['正多面体', '面 F', '棱 E', '顶点 V', 'V − E + F']) thead.appendChild(el('th', null, h));
  ...
  const session = {
    destroy() {
      destroyed = true;
      clearTimeout(toastTimer);
      try { deps.audio?.stopVoice?.(); } catch { /* 静默 */ }
      container.innerHTML = '';
