...
import { cube, box, prism, pyramid, cylinder, cone, frustum, sphere } from '../geo/solids.js';
import { measure } from '../geo/measure.js';
...
.exp-chip {
  min-height: 40px; padding: 0 14px; font-size: 14px; border-radius: 99px;
  border: 1px solid #c3cfdf; background: #fff; color: #1f2d3d; cursor: pointer;
}
.exp-chip.on { background: #2f6fdd; border-color: #2f6fdd; color: #fff; font-weight: 600; }
...
.exp-slider-val { width: 60px; text-align: right; font-size: 14px; font-weight: 600; color: #1f3a5f; font-variant-numeric: tabular-nums; }
.exp-measure { display: grid; grid-template-columns: repeat(auto-fit, minmax(96px, 1fr)); gap: 10px; margin: 14px 0; }
.exp-mstat { text-align: center; padding: 10px 4px; background: #f4f7fb; border-radius: 12px; border: 1px solid #dde4ee; }
.exp-mstat strong { display: block; font-size: 19px; color: #1f3a5f; font-variant-numeric: tabular-nums; }
.exp-mstat span { font-size: 12px; color: #8a97a8; }
.exp-sec { font-size: 15px; font-weight: 600; color: #1f3a5f; margin: 18px 0 8px; }
.exp-card { background: #f4f7fb; border: 1px solid #dde4ee; border-radius: 12px; padding: 12px 14px; margin-bottom: 10px; }
.exp-card-q { font-size: 15px; line-height: 1.6; }
.exp-card details { margin-top: 8px; }
.exp-card summary { cursor: pointer; color: #2f6fdd; font-size: 14px; min-height: 32px; }
.exp-card-ans { margin-top: 6px; font-size: 14px; line-height: 1.7; background: #fff; border: 1px dashed #c3cfdf; border-radius: 8px; padding: 8px 10px; }
.exp-task { background: #fff7e6; border: 1px solid #f2d9a4; border-radius: 12px; padding: 12px 14px; margin-bottom: 10px; font-size: 14px; line-height: 1.6; }
.exp-task.done { background: #d8f3dc; border-color: #2d6a4f; }
.exp-task-head { display: flex; align-items: flex-start; justify-content: space-between; gap: 8px; }
.exp-task-st { flex-shrink: 0; font-size: 12px; padding: 2px 8px; border-radius: 99px; background: #eef1f6; color: #8a97a8; }
.exp-task.done .exp-task-st { background: #2d6a4f; color: #fff; }
.exp-task-live { font-size: 13px; color: #8a97a8; margin-top: 4px; font-variant-numeric: tabular-nums; }
.exp-hint { font-size: 13px; color: #8a97a8; margin-top: 12px; }
`;
...
const SOLIDS = [
  { id: 'cube', name: '正方体', kind: 'cube', make: (p) => cube(p.a),
    params: [P('a', '棱长 a', 1, 6, 0.05, 2)] },
  { id: 'box', name: '长方体', kind: 'box', make: (p) => box(p.a, p.b, p.c),
    params: [P('a', '长 a', 1, 6, 0.05, 2), P('b', '高 b', 0.5, 6, 0.05, 3), P('c', '宽 c', 1, 6, 0.05, 4)] },
  { id: 'prism', name: '正棱柱', kind: 'prism', make: (p) => prism(p.n, p.r, p.h),
    params: [P('n', '棱数 n', 3, 8, 1, 3), P('r', '底面半径 r', 0.8, 3, 0.05, 1.6), P('h', '高 h', 0.5, 5, 0.05, 2.6)] },
  { id: 'pyramid', name: '正棱锥', kind: 'pyramid', make: (p) => pyramid(p.n, p.r, p.h),
    params: [P('n', '棱数 n', 3, 8, 1, 4), P('r', '底面半径 r', 0.8, 3, 0.05, 1.8), P('h', '高 h', 0.5, 5, 0.05, 2.6)] },
  { id: 'cylinder', name: '圆柱', kind: 'cylinder', make: (p) => cylinder(p.r, p.h),
    params: [P('r', '底面半径 r', 0.5, 3, 0.05, 1.4), P('h', '高 h', 0.5, 5, 0.05, 2.6)] },
  { id: 'cone', name: '圆锥', kind: 'cone', make: (p) => cone(p.r, p.h),
    params: [P('r', '底面半径 r', 0.5, 3, 0.05, 1.6), P('h', '高 h', 0.5, 5, 0.05, 2.6)] },
  { id: 'frustum', name: '圆台', kind: 'frustum', make: (p) => frustum(p.r1, p.r2, p.h),
    params: [P('r1', '下底半径 r₁', 0.5, 3, 0.05, 1.6), P('r2', '上底半径 r₂', 0.2, 3, 0.05, 0.9), P('h', '高 h', 0.5, 5, 0.05, 2.2)] },
  { id: 'sphere', name: '球', kind: 'sphere', make: (p) => sphere(p.r),
...
const CARDS = {
  cube: [
    { q: '把棱长 a 加倍，体积变几倍？',
      ans: (p, m) => `现在 V = ${f2(m.volume)}；a 加倍后 V′ = ${f2(8 * m.volume)}，是原来的 <strong>8 倍</strong>——体积随棱长的三次方增长。` },
    { q: '表面积呢？也加倍吗？',
      ans: (p, m) => `现在 S = ${f2(m.surface)}；a 加倍后 S′ = ${f2(4 * m.surface)}，只变 <strong>4 倍</strong>——表面积随棱长的平方增长。` },
  ],
  box: [
    { q: '固定 a、b 不变，把 c 加倍，体积怎么变？',
      ans: (p, m) => `现在 V = abc = ${f2(m.volume)}；c 加倍后 V′ = ${f2(2 * m.volume)}，恰好是原来的 <strong>2 倍</strong>——体积与 c 成正比。` },
    { q: '体对角线 d 和 a、b、c 是什么关系？',
      ans: (p, m) => `d = √(a² + b² + c²) = √(${f2(p.a)}² + ${f2(p.b)}² + ${f2(p.c)}²) = <strong>${f2(m.diagonal)}</strong>。拖动滑杆验证吧。` },
  ],
  prism: [
    { q: '保持底面不变，把高 h 加倍，体积怎么变？',
      ans: (p, m) => `V = S底·h。现在 S底 = ${f2(m.baseArea)}，V = ${f2(m.volume)}；h 加倍后 V′ = ${f2(2 * m.volume)}——<strong>体积与高成正比</strong>。` },
    { q: '把 n 从 3 一路调到 8，棱柱越来越像什么？',
      ans: (p, m) => `像<strong>圆柱</strong>！现在底面积 = ${f2(m.baseArea)}，同半径圆面积 πr² = ${f2(Math.PI * p.r * p.r)}，比值 ${f2(m.baseArea / (Math.PI * p.r * p.r))}——n 越大越接近 1。` },
  ],
  pyramid: [
    { q: '保持底面和高不变，把顶点 P 左右平移，体积会变吗？',
      ans: (p, m) => `<strong>不会</strong>。V = S底·h / 3 只取决于底面积和高：现在 V = ${f2(m.baseArea)} × ${f2(p.h)} / 3 = ${f2(m.volume)}。等底等高的棱锥体积相等。` },
    { q: '同底等高的棱柱，体积是棱锥的几倍？',
      ans: (p, m) => `棱柱 V柱 = S底·h = ${f2(3 * m.volume)}，棱锥 V锥 = ${f2(m.volume)}，比值 = <strong>3.00</strong>。改 n、r、h 都不会改变这个倍数。` },
  ],
  cylinder: [
    { q: 'r 加倍（h 不变），体积变几倍？',
      ans: (p, m) => `V = πr²h：现在 V = ${f2(m.volume)}；r 加倍后 V′ = ${f2(4 * m.volume)}，变 <strong>4 倍</strong>——体积随半径的平方增长。` },
    { q: '侧面展开是什么形状？边长各是多少？',
      ans: (p, m) => `<strong>矩形</strong>：一边是高 h = ${f2(p.h)}，另一边是底面周长 2πr = ${f2(2 * Math.PI * p.r)}，所以 S侧 = ${f2(m.lateral)}。` },
  ],
  cone: [
    { q: '同底等高的圆柱，体积是圆锥的几倍？拖一拖 r、h 验证。',
      ans: (p, m) => `V柱 = πr²h = ${f2(3 * m.volume)}，V锥 = ${f2(m.volume)}，比值 = <strong>3.00</strong>——无论怎么拖，倍数始终是 3。` },
    { q: '母线 l 怎么从 r、h 算出来？',
      ans: (p, m) => `轴截面是直角三角形：l = √(r² + h²) = √(${f2(p.r)}² + ${f2(p.h)}²) = <strong>${f2(m.slant)}</strong>。` },
  ],
  frustum: [
    { q: '把 r₂ 慢慢调向 0，圆台变成什么？体积呢？',
      ans: (p, m) => `变成<strong>圆锥</strong>。r₂→0 时 V → πr₁²h/3 = ${f2(Math.PI * p.r1 * p.r1 * p.h / 3)}；现在 V = ${f2(m.volume)}。` },
    { q: '那把 r₂ 调到等于 r₁ 呢？',
      ans: (p, m) => `变成<strong>圆柱</strong>：V → πr₁²h = ${f2(Math.PI * p.r1 * p.r1 * p.h)}。圆台公式在两种极限下分别退化为圆锥和圆柱公式。` },
  ],
  sphere: [
    { q: '半径加倍，体积变几倍？',
      ans: (p, m) => `V = (4/3)πr³：现在 V = ${f2(m.volume)}；r 加倍后 V′ = ${f2(8 * m.volume)}，变 <strong>8 倍</strong>。` },
    { q: '半径加倍，表面积变几倍？',
      ans: (p, m) => `S = 4πr²：现在 S = ${f2(m.surface)}；r 加倍后 S′ = ${f2(4 * m.surface)}，变 <strong>4 倍</strong>。` },
  ],
};
...
const TASKS = [
  ...
  { id: 't-box', solid: 'box',
    ...
    done: (p, m) => Math.abs(p.a - 2) < 0.03 && Math.abs(p.b - 3) < 0.03 && Math.abs(m.volume - 18) < 0.1,
    live: (p, m) => `当前 a = ${f2(p.a)}，b = ${f2(p.b)}，V = ${f2(m.volume)}` },
  { id: 't-cylinder', solid: 'cylinder',
...
export function initExplore(container, deps = {}) {
  ...
  root.appendChild(el('p', 'exp-hint', '拖动滑杆改变参数，3D 模型与几何量实时联动；任务达成自动判对。'));
  ...
  function buildCards() {
    ...
    for (const c of CARDS[state.solidId] || []) {
      ...
      card.appendChild(el('div', 'exp-card-q', `💭 ${c.q}`));
  ...
  function buildTasks() {
    ...
    for (const t of TASKS) {
      ...
      head.appendChild(el('span', null, `🎯 ${t.text}`));
  ...
  function update() {
    ...
    const addStat = (label, v) => {
      if (v == null) return;
      measureBox.appendChild(el('div', 'exp-mstat', `<strong>${fmt(v)}</strong><span>${label}</span>`));
    };
    addStat('体积 V', m.volume);
    addStat('表面积 S', m.surface);
    addStat('侧面积 S侧', m.lateral);
    addStat(state.solidId === 'box' || state.solidId === 'cube' ? '体对角线 d' : '母线 l',
    ...
    for (const card of tasksBox.children) {
      ...
      st.textContent = done ? '✓ 已完成' : active ? '探究中' : `切换到${SOLIDS.find((s) => s.id === t.solid).name}`;
