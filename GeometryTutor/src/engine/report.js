...
export function buildReport(data, day) {
  ...
  let weakCat = null, weakWrong = 0;
  for (const [c, n] of Object.entries(wrongByCat)) {
    if (n > weakWrong) { weakCat = c; weakWrong = n; }
  }

  const mastery = data.mastery && typeof data.mastery === 'object' ? data.mastery : {};
  const stars = categories
    .map((cat) => ({ cat, level: levelOf(mastery, cat), stars: starsOf(mastery, cat) }));
  ...
  return {
    weekStart: start, weekEnd: day,
    days, currentStreak: current,
    lessonsDone: data.lessonsDone | 0, lessonsTotal: data.lessonsTotal | 0,
    answers, acc, conquered,
    weakCat, weakWrong,
    stars,
    suggestion,
  };
