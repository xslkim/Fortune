...
export function emptyStreak() {
  return { current: 0, best: 0, lastDoneDate: null, freezes: FREEZES_PER_MONTH, freezeMonth: null, today: null };
}

function norm(streak) {
  const s = streak && typeof streak === 'object' ? streak : {};
  return {
    current: s.current | 0, best: s.best | 0,
...
export function recordActivity(streak, kind, day) {
  ...
  if (s.freezeMonth !== month) { s.freezes = FREEZES_PER_MONTH; s.freezeMonth = month; } // 月初重置

  const today = s.today && s.today.date === day ? { ...s.today } : { date: day, steps: 0, answers: 0, games: 0 };
  if (kind === 'step') today.steps += 1;
  else if (kind === 'answer') today.answers += 1;
  else if (kind === 'game') today.games = (today.games | 0) + 1;
  s.today = today;

  const goalMet = today.steps >= GOAL_STEPS || today.answers >= GOAL_ANSWERS || (today.games | 0) >= GOAL_GAMES;
  if (goalMet && s.lastDoneDate !== day) {
    if (s.lastDoneDate === null) {
      ...
      else if (gap === 2 && s.freezes > 0) { s.freezes -= 1; s.current += 1; } // 断签保护
...
export function goalProgress(streak, day) {
  ...
  return { steps: t.steps | 0, answers: t.answers | 0, games: t.games | 0, done: s.lastDoneDate === day };
}
...
export function saveStreak(s) {
  try { localStorage.setItem(KEY, JSON.stringify(s)); } catch { /* 静默 */ }
}
