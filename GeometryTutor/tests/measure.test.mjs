...
import { measure, meshVolume, meshSurfaceArea, meshLateralArea, regularNGon } from '../src/geo/measure.js';

const close = (actual, expected, tol, msg) => {
  const rel = Math.abs(actual - expected) / Math.abs(expected);
  assert.ok(rel < tol, `${msg}: 网格值 ${actual} vs 公式值 ${expected}，相对误差 ${(rel * 100).toFixed(3)}%`);
};
