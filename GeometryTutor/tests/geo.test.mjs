...
import { buildEdges, classifyFaces, classifyEdges, sectionPolygon, normFace, faceNormal } from '../src/geo/topology.js';
...
test('所有面法线朝外（CCW 约定正确）', () => {
  for (const item of catalog()) {
    ...
    g.faces.forEach((face, i) => {
      ...
      const c = [0, 0, 0];
      for (const ix of f.idx) { c[0] += g.points[ix].p[0]; c[1] += g.points[ix].p[1]; c[2] += g.points[ix].p[2]; }
      c[0] /= f.idx.length; c[1] /= f.idx.length; c[2] /= f.idx.length;
