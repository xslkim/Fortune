...
function assertCoplanar(g, mats, tol = 1e-6) {
  ...
  for (let fi = 0; fi < g.faces.length; fi++) {
    ...
    assert.ok(Math.abs(dot([stats[fi].center[0] - c0[0], stats[fi].center[1] - c0[1], stats[fi].center[2] - c0[2]], n0)) < tol,
      `面 ${fi} 面心不在展开平面内`);
  }
  return stats;
}
...
test('球不支持展开，其余目录几何体均支持', () => {
  ...
  for (const g of [cube(), box(), prism(3), prism(6), pyramid(3), pyramid(4), cylinder(), cone(), frustum()]) {
...
test('圆柱 t=1：侧面摊成近似长方形（共面，宽度 ≈ 2πr ±5%），底面成圆盘挂侧面边上', () => {
  ...
  for (const v of sideVerts) {
    assert.ok(Math.abs(dot([v[0] - v0[0], v[1] - v0[1], v[2] - v0[2]], n)) < 1e-4, '侧面顶点不共面');
  }

  // 展开宽度：沿条带方向（面内 ⊥ 原圆柱轴线 y）的跨度
  const e = norm(cross([0, 1, 0], n));
  let lo = Infinity, hi = -Infinity;
  for (const v of sideVerts) { const d = dot(v, e); lo = Math.min(lo, d); hi = Math.max(hi, d); }
  const width = hi - lo;
  assert.ok(Math.abs(width - 2 * Math.PI * r) / (2 * Math.PI * r) < 0.05,
