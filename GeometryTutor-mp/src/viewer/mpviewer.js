...
import { buildEdges, classifyFaces, classifyEdges, sectionPolygon, normFace } from '../geo/topology.js';
import { unfoldLayout, applyPoint } from '../geo/unfold.js';
import { sub } from '../geo/solids.js';
...
export class GeoViewerMP {
  ...
  setSolid(data) {
    ...
    this.matEdgeBack = new THREE.LineDashedMaterial({
      color: COLORS.edgeBack, dashSize: 0.09, gapSize: 0.07, transparent: true, opacity: 0.9,
    });
    ...
    this.faceGroups = data.faces.map((face) => {
      ...
      const matFront = new THREE.MeshLambertMaterial({
        color: COLORS.faceFront, transparent: true, opacity: 0.6, side: THREE.FrontSide,
      });
      const matBack = new THREE.MeshLambertMaterial({
        color: COLORS.faceBack, transparent: true, opacity: 0.28, side: THREE.BackSide,
      });
      const front = new THREE.Mesh(geo, matFront);
      const back = new THREE.Mesh(geo, matBack);
      const border = new THREE.LineLoop(
        new THREE.BufferGeometry().setFromPoints(idx.map((i) => {
    ...
    this.edgeLines = this.edges.map((e) => {
      const pa = data.points[e.a].p, pb = data.points[e.b].p;
    ...
    data.faces.forEach((face, fi) => {
      for (const i of normFace(face).idx) {
        ...
        if (!q.label) continue;
        const marker = new THREE.Mesh(sphereGeo, new THREE.MeshBasicMaterial({ color: COLORS.point }));
        marker.position.set(q.p[0], q.p[1], q.p[2]);
  ...
  highlight(h = {}) {
    ...
    for (const [la, lb] of edges) {
      const ia = labelToIndex.get(la), ib = labelToIndex.get(lb);
      if (ia == null || ib == null) continue;
      const pa = this.solid.points[ia].p, pb = this.solid.points[ib].p;
  ...
  setSection(plane) {
    ...
    geo.setAttribute('position', new THREE.Float32BufferAttribute(pos, 3));
    geo.computeVertexNormals();
    this.sectionMesh = new THREE.Mesh(geo, new THREE.MeshBasicMaterial({
      color: COLORS.section, transparent: true, opacity: 0.65,
  ...
  _suggestUnfoldView() {
    ...
    this.phi = Math.min(this.maxPhi, Math.max(this.minPhi, Math.acos(Math.min(1, Math.max(-1, n[1])))));
  ...
  _updateLabels() {
    ...
    if (this.unfoldT > 0) {
      ...
      for (const pm of this.pointMarkers) {
        ...
        V.project(this.camera);
        if (V.z > 1) continue;
        const hidden = this._facing && this.pointFaces[pm.index].every((f) => this._facing[f] === 'back');
        labels.push({
          key: `p${pm.index}`,
          text: sub(q.label),
          x: ((V.x + 1) / 2) * w,
  ...
  dispose() {
    this._disposed = true;
    try { this._win.cancelAnimationFrame.call(this.canvas, this._raf); } catch { /* 忽略 */ }
    this._clearSolid();
